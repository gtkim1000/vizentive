(() => {
  ...
  style.textContent += '.admin-table{min-width:1900px}.admin-sort{display:inline-flex;align-items:center;gap:6px;border:0;background:none;padding:0;color:inherit;font:inherit;font-weight:inherit;cursor:pointer;white-space:nowrap}.admin-sort span{color:#7657f6;font-size:.72rem}.admin-sort:focus-visible{outline:2px solid #7657f6;outline-offset:4px;border-radius:3px}';
  ...
  menu.addEventListener('click', async event => {
    ...
    overlay.innerHTML = `<div class="admin-shell"><header class="admin-head"><div><small>VIZENTIVE ADMIN</small><h2>상담 기록</h2><span class="admin-count"></span></div><button class="admin-close" type="button">신청 화면으로 돌아가기</button></header><div class="admin-tools"><input type="search" placeholder="업체명, 이름, 전화번호, 이메일 검색"><select><option value="">전체 상태</option><option value="new">신규</option><option value="contacting">연락 중</option><option value="in_progress">진행 중</option><option value="completed">완료</option><option value="cancelled">취소</option></select></div><div class="admin-layout"><div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>상담 상태</th><th aria-sort="none"><button class="admin-sort" type="button" data-sort="created_at">접수일시 <span>↕</span></button></th><th>희망 연락 방식</th><th aria-sort="none"><button class="admin-sort" type="button" data-sort="brand">회사·브랜드명 <span>↕</span></button></th><th aria-sort="none"><button class="admin-sort" type="button" data-sort="contact_name">담당자명 <span>↕</span></button></th><th aria-sort="none"><button class="admin-sort" type="button" data-sort="phone">전화번호 <span>↕</span></button></th><th>업종</th><th>이메일</th><th>신청 서비스</th><th>예상 제작 수량</th><th>희망 일정</th><th>문의 내용</th><th>개인정보 동의</th></tr></thead><tbody></tbody></table></div><aside class="admin-detail" aria-live="polite"></aside></div></div>`;
    document.body.append(overlay);
    document.body.style.overflow = 'hidden';
    const input = overlay.querySelector('input'), select = overlay.querySelector('select'), thead = overlay.querySelector('thead'), tbody = overlay.querySelector('tbody'), count = overlay.querySelector('.admin-count'), detail = overlay.querySelector('.admin-detail');
    ...
    const renderDetail = () => {
      ...
      detail.innerHTML = `<h3>${esc(row.contact_name)} · ${esc(row.brand)}</h3><div class="admin-detail-id">${esc(row.created_at ? new Date(row.created_at).toLocaleString('ko-KR') : '미입력')}</div><div class="admin-detail-grid"><div class="admin-field"><label>상담 상태</label><div><span class="admin-status">${esc(status(row.status))}</span></div></div><div class="admin-field"><label>접수일시</label><div>${esc(row.created_at ? new Date(row.created_at).toLocaleString('ko-KR') : '미입력')}</div></div><div class="admin-field full"><label>희망 연락 방식</label><div>${esc(channel(row.channel))}</div></div><div class="admin-field"><label>회사·브랜드명</label><div>${esc(row.brand || '미입력')}</div></div><div class="admin-field"><label>담당자명</label><div>${esc(row.contact_name || '미입력')}</div></div><div class="admin-field"><label>전화번호</label><div>${row.phone ? `<a href="tel:${esc(row.phone)}">${esc(row.phone)}</a>` : '미입력'}</div></div><div class="admin-field"><label>업종</label><div>${esc(row.industry || '미입력')}</div></div><div class="admin-field full"><label>이메일</label><div>${row.email ? `<a href="mailto:${esc(row.email)}">${esc(row.email)}</a>` : '미입력'}</div></div><div class="admin-field full"><label>신청 서비스</label><div>${esc((row.services || []).join(', ') || '미입력')}</div></div><div class="admin-field"><label>예상 제작 수량</label><div>${esc(row.quantity || '미입력')}</div></div><div class="admin-field"><label>희망 일정</label><div>${esc(row.desired_date || '미입력')}</div></div><div class="admin-field full"><label>문의 내용</label><div>${esc(row.message || '미입력')}</div></div><div class="admin-field full"><label>개인정보 수집 동의</label><div>${row.consent === true ? '동의' : row.consent === false ? '미동의' : '미입력'}</div></div></div>`;
    };
    const render = () => {
      ...
      if (sortKey) visibleRows.sort((a,b) => { const left = sortKey === 'created_at' ? new Date(a[sortKey] || 0).getTime() : String(a[sortKey] || ''), right = sortKey === 'created_at' ? new Date(b[sortKey] || 0).getTime() : String(b[sortKey] || ''); return (typeof left === 'number' ? left - right : left.localeCompare(right,'ko',{numeric:true,sensitivity:'base'})) * (sortDirection === 'asc' ? 1 : -1); });
      thead.querySelectorAll('.admin-sort').forEach(button => { const active = button.dataset.sort === sortKey, th = button.closest('th'); th.setAttribute('aria-sort', active ? (sortDirection === 'asc' ? 'ascending' : 'descending') : 'none'); button.querySelector('span').textContent = active ? (sortDirection === 'asc' ? '▲' : '▼') : '↕'; });
      ...
      tbody.innerHTML = visibleRows.length ? visibleRows.map(row => `<tr data-id="${esc(row.receipt_id)}" class="${row.receipt_id === selectedId ? 'selected' : ''}" tabindex="0"><td>${esc(status(row.status))}</td><td>${esc(row.created_at ? new Date(row.created_at).toLocaleString('ko-KR') : '미입력')}</td><td>${esc(channel(row.channel))}</td><td>${esc(row.brand || '미입력')}</td><td>${esc(row.contact_name || '미입력')}</td><td>${esc(row.phone || '미입력')}</td><td>${esc(row.industry || '미입력')}</td><td>${esc(row.email || '미입력')}</td><td>${esc((row.services || []).join(', ') || '미입력')}</td><td>${esc(row.quantity || '미입력')}</td><td>${esc(row.desired_date || '미입력')}</td><td>${esc(row.message || '미입력')}</td><td>${row.consent === true ? '동의' : row.consent === false ? '미동의' : '미입력'}</td></tr>`).join('') : '<tr><td class="admin-empty" colspan="13">조건에 맞는 상담 기록이 없습니다.</td></tr>';
      renderDetail();
    };
    ...
    thead.addEventListener('click', event => { const button = event.target.closest('.admin-sort'); if (!button) return; sortDirection = sortKey === button.dataset.sort && sortDirection === 'asc' ? 'desc' : 'asc'; sortKey = button.dataset.sort; render(); });
