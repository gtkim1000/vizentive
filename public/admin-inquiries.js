(() => {
  ...
  menu.addEventListener('click', async event => {
    ...
    const renderDetail = () => {
      ...
      detail.innerHTML = `<h3>${esc(row.contact_name)} · ${esc(row.brand)}</h3><div class="admin-detail-id">${esc(row.receipt_id)} · ${esc(new Date(row.created_at).toLocaleString('ko-KR'))}</div><div class="admin-detail-grid"><div class="admin-field"><label>상담 상태</label><div><span class="admin-status">${esc(status(row.status))}</span></div></div><div class="admin-field"><label>희망 연락 방식</label><div>${esc(channel(row.channel))}</div></div><div class="admin-field"><label>담당자명</label><div>${esc(row.contact_name)}</div></div><div class="admin-field"><label>회사·브랜드명</label><div>${esc(row.brand)}</div></div><div class="admin-field"><label>전화번호</label><div><a href="tel:${esc(row.phone)}">${esc(row.phone)}</a></div></div><div class="admin-field"><label>이메일</label><div><a href="mailto:${esc(row.email)}">${esc(row.email)}</a></div></div><div class="admin-field"><label>업종</label><div>${esc(row.industry)}</div></div><div class="admin-field"><label>희망 일정</label><div>${esc(row.desired_date || '미정')}</div></div><div class="admin-field full"><label>신청 서비스</label><div>${esc((row.services || []).join(', ') || '-')}</div></div><div class="admin-field full"><label>예상 제작 수량</label><div>${esc(row.quantity || '미정')}</div></div><div class="admin-field full"><label>문의 내용</label><div>${esc(row.message || '-')}</div></div></div>`;
    };
    const render = () => {
      ...
      tbody.innerHTML = visibleRows.length ? visibleRows.map(row => `<tr data-id="${esc(row.receipt_id)}" class="${row.receipt_id === selectedId ? 'selected' : ''}" tabindex="0"><td>${esc(new Date(row.created_at).toLocaleDateString('ko-KR'))}<br><small>${esc(row.receipt_id)}</small></td><td>${esc(status(row.status))}</td><td><strong>${esc(row.contact_name)}</strong><br>${esc(row.brand)}</td><td>${esc(row.phone)}<br><small>${esc(channel(row.channel))}</small></td><td>${esc((row.services || []).join(', '))}</td></tr>`).join('') : '<tr><td class="admin-empty" colspan="5">조건에 맞는 상담 기록이 없습니다.</td></tr>';
      renderDetail();
    };
